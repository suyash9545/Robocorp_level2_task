...
Merge the reciept and PDF
    [Arguments]         ${screenshot}    ${pdf}    ${order_number}
    Log                 ${pdf}
    Open Pdf            ${pdf}
    ${files}=    Create List
    ...    ${pdf}
    ...    ${screenshot}
    Add files to pdf    ${files}    ${CURDIR}${/}result${/}robot-${order_number}.pdf
    # Close Pdf           ${pdf}
...
Order new Robot
    ...
    Merge the reciept and PDF       ${screenshot}    ${pdf}    ${order}[Order number]
...
*** Keywords ***
Creat ZIP File
    Archive Folder With Zip    ${CURDIR}${/}result${/}    ${CURDIR}${/}orders.zip
